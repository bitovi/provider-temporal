...
import (
	...
	apisCluster "github.com/bitovi/provider-temporal/apis/cluster"
	apisNamespaced "github.com/bitovi/provider-temporal/apis/namespaced"
	"github.com/bitovi/provider-temporal/config"
	"github.com/bitovi/provider-temporal/internal/clients"
	controllerCluster "github.com/bitovi/provider-temporal/internal/controller/cluster"
	controllerNamespaced "github.com/bitovi/provider-temporal/internal/controller/namespaced"
	"github.com/bitovi/provider-temporal/internal/features"
	"github.com/bitovi/provider-temporal/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for TemporalCloud").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-temporalcloud"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-temporalcloud",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped TemporalCloud APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced TemporalCloud APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped TemporalCloud controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced TemporalCloud controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped TemporalCloud controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced TemporalCloud controllers")
	}
