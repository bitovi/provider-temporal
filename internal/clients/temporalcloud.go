...
import (
	...
	clusterv1beta1 "github.com/bitovi/provider-temporal/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/bitovi/provider-temporal/apis/namespaced/v1beta1"
)
...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal temporalcloud credentials as JSON"
	errMissingAPIKey        = "credentials must contain api_key (JSON) or be a raw API key"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		apiKey, err := extractAPIKey(data)
		if err != nil {
			return ps, err
		}
		// temporalio/temporalcloud provider schema key is api_key.
...
func extractAPIKey(data []byte) (string, error) {
	...
	if v := creds["TEMPORAL_CLOUD_API_KEY"]; v != "" {
		return v, nil
	}
	return "", errors.New(errMissingAPIKey)
}
