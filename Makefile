...
PROJECT_NAME ?= provider-temporalcloud
# Go module / PROJECT_REPO = GitHub repo path. Package/image name stays provider-temporalcloud.
PROJECT_REPO ?= github.com/bitovi/provider-temporal

# common.mk computes VERSION from git; a repo with no commits has no HEAD.
ifeq ($(shell git rev-parse --verify HEAD >/dev/null 2>&1 && echo ok),)
...
export TERRAFORM_PROVIDER_REPO ?= https://github.com/temporalio/terraform-provider-temporalcloud
export TERRAFORM_PROVIDER_VERSION ?= 1.6.0
export TERRAFORM_PROVIDER_DOWNLOAD_NAME ?= terraform-provider-temporalcloud
export TERRAFORM_PROVIDER_DOWNLOAD_URL_PREFIX ?= https://github.com/temporalio/terraform-provider-temporalcloud/releases/download/v$(TERRAFORM_PROVIDER_VERSION)
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-temporalcloud_v1.6.0
...
# Packages are only published to Bitovi GHCR (see .github/workflows/publish-ghcr.yml).
# Skip channel promote tags; version tags only.
XPKG_REG_ORGS_NO_PROMOTE ?= ghcr.io/bitovi
XPKGS = $(PROJECT_NAME)
...
xpkg.build.provider-temporalcloud: do.build.images
